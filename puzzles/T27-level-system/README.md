...
A pure-TLA+ spec for a **lock** sits in `solution/Lock.tla`. It declares several named formulas: `TypeOK`, `Locked`, `LockIt`, `Unlock`, `Stable`, `EventuallyUnlocked`, `NeverDoubleLocked`, `Init`, `Next`, `Spec`. Some are state-level, some action-level, some temporal-level.
