...
A 105-puzzle progressive curriculum for learning **TLA+** and **PlusCal** by doing — small, self-contained exercises that each teach one new concept and verify with `tlc` (and, where annotated, `apalache`).
...
- **Tier 0 — Prelude (4):** the toolchain. Run TLC, read a counterexample, edit a `.cfg`, understand what `tlc -pcal` does.
