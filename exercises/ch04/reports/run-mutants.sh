#!/usr/bin/env bash
# Run every seeded ch04 mutant through harness/verdict.sh and print one row
# per mutant: id, module, verdict token, raw exit status.
#
# Usage:  bash exercises/ch04/reports/run-mutants.sh [MUT_DIR]
#
# Run it from the repo root, after reports/mutants.py has seeded MUT_DIR.
# MUT_DIR defaults to `.ch04-mut`, matching the seeder's default.
#
# PlusCal mutants are re-translated first. TLC checks the translation and not
# the algorithm comment, so an edit inside the PlusCal block does nothing
# until pcal has run again.
#
