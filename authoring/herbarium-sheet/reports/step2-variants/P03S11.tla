...
\* Variant P03S11 (only-grows-blind-to-a-replacement) of the herbarium-sheet reference.
...
File(b, s, n) ==
    /\ reading[b][s] # None
    /\ LET filed == {[name |-> n, stamp |-> reading[b][s]]}
          ...
          /\ \A b \in Botanists :
                (/\ Observe'.reading[b][s] # None
                 /\ Observe'.reading[b][s] # Observe.reading[b][s])
                    => /\ Observe'.consulted[s] # Observe.consulted[s]
                       /\ Observe'.consulted[s] = Observe'.reading[b][s]]_(Observe.consulted)
