...
Sweep(m) ==
    /\ \E p \in Pieces : stage[p] = "floor" /\ modification[p] < UpperMark
    /\ modification' = [p \in Pieces |->
           IF stage[p] = "floor" /\ modification[p] < UpperMark
           THEN modification[p] + 1
           ELSE modification[p]]
    /\ UNCHANGED stage
          ...
          (/\ p # q
           /\ \/ Observe'.stage[p] # Observe.stage[p]
              ...
              => /\ Observe'.stage[q] = Observe.stage[q]
                 /\ Observe'.modification[q] = Observe.modification[q]]_(Observe.stage)
