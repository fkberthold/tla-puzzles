...
- **The book entry**: a stored number, or a change history the standing
  number derives from. The book field shows the number either way. (What a
  change step may do is fixed by Rule 2: any new value in one step. A model
  that only moves an entry by one unit at a time is modeling a different
  club, and that isn't this fork.)
- **Shares**: stored at placement, or derived from the frozen book and a
  collected flag.
- **The order**: a recorded total, or derived from the book when needed.
- **The supplier**: a process, or a bare delivery action.
...
1. **The threshold permits, never compels.** The alternative places the order
...
2. **The snapshot binds the standing book, unasked.** The alternative runs a
   ...
   system, and it isn't how a pledge book works. The point of a standing
   pledge is that it stands.
3. **One `Min` for the whole catalog.** Real wholesalers set per-product
   minimums. Per-product constants multiply the config for no new modeling
   question, museum's one-sensitivity-class reasoning again.
4. **Placement closes the book for good.** The alternative takes late pledges
   into a second round for the same product. That's the next cycle wearing
   this cycle's clothes, and Rule 6 puts it out of scope.
5. **Whole-share collection.** Partial pickups are share arithmetic with no
   new judgment in them.
6. **No money.** Payment at pledge time makes the book an escrow, and payment
   at pickup makes the table a till. Either adds a parallel ledger and no new
   question about the book itself, which is the system here.
7. **The supplier delivers whole, once, eventually.** Shorted orders,
   substitutions, and failed deliveries make recovery the subject. Recovery is
   a different problem, the same way museum kept recall out.
8. **`Cap` is the club's rule, stated in the open.** The alternative leaves
   pledges unbounded and invents a bound as a checking device. I'd rather the
   bound be the fairness rule real clubs have than a device the domain never
   heard of.
9. **A zero pledge is just an entry.** A member who pledges two and withdraws
   looks, at the interface, like one who never pledged. No property tells them
   apart, on purpose.
