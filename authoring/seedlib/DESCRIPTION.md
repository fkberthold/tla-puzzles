...
The recall probe liked this domain because more than one model of it is defensible.
These are the choices the description leaves open on purpose. An edit that closes one
is a regression, not a tightening.

**Counts or lots**: stock as per-variety tallies, or tracked lots with year and
provenance. No rule reads past the label (Rule 1), so both fit and neither is nudged.

**Nominal or exact discharge**: a return discharges by label, or a model carries
what's under the label (amount, viability, even mislabels). The library never
verifies (Rule 5), so the richer model projects onto the same observations.

**What failure is**: a standing flag, the persisting debts, or both as primary state.
The rules pin their joint behavior (properties 6 and 7), not which is stored and
which derived.

**The garden**: an explicit grow or harvest step between checkout and return, or
none. The library can't see it, so both are faithful. Under `Observe` a grow step is
stutter.
...
risk. Mine are on the table instead.

1. **Reborrow gate.** Chose: no checkout of a variety you currently owe. Alternative:
   a per-season ration, one packet per variety per member per season. They differ on
   borrow, return, borrow again inside one season (mine allows it), and the ration
   needs season-scoped memory plus one more observable field.
2. **Debt persistence.** Chose: debts survive closes until met, payable in any later
   season. Alternatives: expulsion at a missed deadline, or forgiveness at the close.
   Persistence makes failure a state with an exit, so entering and leaving default
   are both gradable.
3. **Restoration threshold.** Chose: standing returns only when the last debt clears.
   Alternative: privileges per debt. All-clear keeps standing two-valued and keeps
   property 6's close condition crisp.
4. **Within-season time.** Chose: none, and no gap between seasons either. Checkout
   and return are enabled in any order inside a season, so a same-instant turnaround
   is possible, which no garden could do. The library's process can't tell (Rule 4),
   only the calendar could, so I flattened it. Alternative: planting and harvest
   phases inside each season.
5. **Donations.** Chose: excluded. Alternative: an action that adds stock with no
   debt. Including it trades the conservation equality (property 5) for an
   inequality, and adds an action that touches nothing else.
6. **Verification.** Chose: honor system, stated as a rule. Alternative: librarian
   inspection with accept and reject. Trust is what keeps the nominal-or-exact fork
   open, and I think it matches the domain's real character.
7. **The horizon.** Chose: a three-season program with a real end. Alternative: a
   perpetual library. Perpetual loses the end-of-program audit (properties 8 and 9)
   and needs a different finiteness story.
8. **Compulsion.** Chose: the calendar is the one thing that must act, and members
   never must. Alternative: closes that wait on the librarian. Then a model can idle
   forever and every deadline loses its teeth.
